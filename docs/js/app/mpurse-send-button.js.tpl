...
.flip {
  ...
  animation: flip .20s linear alternate;
    ...
    #getImgWidth() { return parseInt( (0 <= this.size.indexOf('x')) ? this.size.split('x')[0] : this.size) }
    #getImgHeight() { return parseInt( (0 <= this.size.indexOf('x')) ? this.size.split('x')[1] : this.size) }
    #parseImgSize() {
        if (0 <= this.size.indexOf('x')) { return this.size.split('x').map(v=>(parseInt(v)) ? parseInt(v) : 64) }
        else { return (parseInt(this.size)) ? [parseInt(this.size), parseInt(this.size)] : [64, 64] }
    }
    #getImgSrc() {
        ...
        if (this.img) {
            ...
            if (isNaN(num)) {
                const key = this.icon.getKey(this.img, this.size)
                ...
                return this.icon.get(this.img, this.size)
            }
        }
